...
@app.get(
    "/admin/enrollment-analytics", dependencies=[Depends(require_role(ROLE_ADMIN))]
)
...
async def signup_for_activity(
    activity_name: str, current_user: UserInDB = Depends(get_current_user)
):
    ...
    if email in activity.get("waitlist", []):
        raise HTTPException(
            status_code=400, detail="Student is already on the waitlist"
        )
    if len(activity.get("participants", [])) >= activity.get("max_participants", 0):
        # Add to waitlist
        await db["activities"].update_one(
            {"name": activity_name}, {"$push": {"waitlist": email}}
        )
        return {
            "message": f"Activity is full. {email} added to waitlist for {activity_name}"
        }
    await db["activities"].update_one(
        {"name": activity_name}, {"$push": {"participants": email}}
    )
...
async def unregister_from_activity(
    ...
    # Remove student
    await db["activities"].update_one(
        {"name": activity_name}, {"$pull": {"participants": email}}
    )
...
@app.post(
    "/groups/{group_name}/assign-student",
...
async def assign_student_to_group(group_name: str, student_email: str = Body(...)):
    ...
    await db["groups"].update_one(
        {"name": group_name}, {"$addToSet": {"students": student_email}}
    )
...
@app.post(
    "/activities/{activity_name}/assign-instructor",
    dependencies=[Depends(require_role(ROLE_ADMIN))],
)
async def assign_instructor_to_activity(
    activity_name: str, instructor_email: str = Body(...)
):
    activity = await db["activities"].find_one({"name": activity_name})
    if not activity:
        raise HTTPException(status_code=404, detail="Activity not found")
    await db["activities"].update_one(
        {"name": activity_name}, {"$set": {"instructor": instructor_email}}
    )
    return {
        "message": f"Assigned instructor {instructor_email} to activity {activity_name}"
    }
